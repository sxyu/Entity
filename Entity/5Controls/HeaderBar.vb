...
Namespace _5Controls
    ...
    Public Class HeaderBar
        ...
        Private Sub topBar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            ...
            btnAbout.Location = New Point(btnMin.Left - 61, 9)
            btnSetup.Location = New Point(btnAbout.Left - 40, 9)
            btnAud.Location = New Point(btnSetup.Left - 61, 9)
            ...
            If My.Settings.timeFormat12 Then
                nowStr = CStr(Now)
            Else
                nowStr = CStr(Now.Date) & " " & Now.Hour.ToString & ":" & r2Dgts(Now.Minute.ToString) & ":" &
                         r2Dgts(Now.Second.ToString)
            End If
            mainTT.SetToolTip(lbTime, nowStr & vbCrLf & "Click to toggle 24H / 12H formats.")
        ...
        Private Sub UpdateTooltipText(str As String)
            If str.Length > 0 Then mainTT.SetToolTip(lbTime, str)
        End Sub
        ...
        Private Sub timer_Elapsed(sender As Object, e As EventArgs)
            ...
            Else
                nowStr = CStr(Now.Date) & " " & Now.Hour.ToString & ":" & R2Dgts(Now.Minute.ToString) & ":" &
                         R2Dgts(Now.Second.ToString)
            End If
            nowStr &= vbCrLf & "Click to toggle 24H / 12H formats."
            If InvokeRequired Then
                BeginInvoke(New SafeTooltipDelegate(AddressOf updateTooltipText), nowStr)
            Else
                mainTT.SetToolTip(lbTime, nowStr)
            End If
        End Sub
