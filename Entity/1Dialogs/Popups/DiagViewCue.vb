...
Imports Cue = Entity._2Namespaces.CueSys
Imports Entity._2Namespaces

Namespace _1Dialogs.Popups
    Public Class DiagViewCue
        Inherits Form

        Public StartingCue As String = "" 'Cue to start with
        Public SelectCue As String = "" 'Cue to select

        Dim omittedCueList As New List(Of String)
        Dim _forceAddLast As Boolean = False
        Dim _noAutoScroll As Boolean = False
        Dim curTab As CuesTab = CuesTab.Lfx
        ...
        Private Sub DiagViewCue_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
            If IsPresMode Then
                e.Cancel = True
                Frm5ShowUi.pnlCue.Invalidate()
                Me.Hide()
            End If
        End Sub
        Private Sub DiagViewCue_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            'docks in viewer
        ...
        Protected Overrides Sub WndProc(ByRef m As System.Windows.Forms.Message)
            ...
            End If
        End Sub
        Public Sub New(ByVal _startingCue As String, ByVal _selectCue As String)

            ' This call is required by the designer.
            InitializeComponent()

            ' Add any initialization after the InitializeComponent() call.
            Me.StartingCue = _startingCue
            Me.SelectCue = _selectCue
        End Sub
        ...
        Private Sub loadLv()
            ...
            omittedCueList.Clear()
            For Each c As _2Namespaces.CueSys.Cue In Cues
                If c.Name = StartingCue Then started = True
                If started Then
                    ...
                    toAdd.Add(it)
                Else
                    omittedCueList.Add(c.Name)
                End If
            Next
            'UpdateVisual()
            lv.Items.AddRange(toAdd.ToArray)
            If lv.Items.Count > 0 Then
                If Not String.IsNullOrEmpty(SelectCue) Then
                    For Each i As ListViewItem In lv.Items
                        If i.Text = SelectCue Then
                            i.Selected = True
                            Exit For
                        End If
                    Next
        ...
        Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
            ...
            If lv.Items.Count > 0 Then
                ...
                End If
            Else
                Cues.Add(New _2Namespaces.CueSys.Cue(it.Text, True, True, True))
            ...
            it.Selected = True
            updateActiveRects()
            UpdateCueOrder(lv, omittedCueList.Count)
            SaveCues()
            'CheckCueNum()
        End Sub
            ...
            If lv.SelectedItems.Count = 1 Then
                If tbName.Visible Then
                    ...
                    If e.Button = Windows.Forms.MouseButtons.Left And tbName.Text <> lbName.Text Then
                        ...
                        Dim cCue As CueSys.Cue = GetCueByName(oldName)
        ...
        Private Sub lv_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lv.SelectedIndexChanged
            ...
            Else
                ...
                Dim curCue As CueSys.Cue = GetCueByName(lv.SelectedItems(0).Text)
                ...
                Else
                    Using drawBmp As Bitmap = GetThumbnail(ImgResToPath(curPjxCue.NameOfRes), pbPjx.Width, pbPjx.Height, _
                                                           Color.FromArgb(160, 160, 160))
                        pbPjx.Image = New Bitmap(drawBmp)
                    End Using
                End If
            ...
            lv.Refresh()
        End Sub
        Private Function GetCurCue() As CueSys.Cue
        ...
        Private Sub btnDel_Click(sender As Object, e As EventArgs) Handles btnDel.Click
            ...
            For i As Integer = 0 To lv.SelectedItems.Count - 1
                ...
                End If
            Next
            If MsgBox("Delete the cue" & s & "''" & resNames & "''?", MsgBoxStyle.YesNo _
                                                                      Or MsgBoxStyle.MsgBoxSetForeground, "Delete Cue" & s) = MsgBoxResult.Yes Then
        ...
        Private Sub channelsView_SelectedIndexChanged(sender As Object, e As EventArgs) Handles asngLV.SelectedIndexChanged
            ...
            If asngLV.SelectedItems.Count <> 1 Then
                ...
                Else
                    ...
                    If _noAutoScroll Then
                        _noAutoScroll = False
                    Else
                        editor.ScrollControlIntoView(bottomDummy)
                    End If
                End If
            Else
                If _noAutoScroll Then
                ...
                Else
                    ...
                    For Each l As Light In Lights
                        ...
                        If l.Name = asngLV.SelectedItems(0).Text.Remove(0, asngLV.SelectedItems(0).Text.IndexOf(":") + 1).Trim Then
                    ...
                    For i As Integer = 0 To cbLLight.Items.Count - 1
                        If cbLLight.Items(i).ToString = asngLV.SelectedItems(0).Text.Remove(0, asngLV.SelectedItems(0).Text.IndexOf(":") + 1).Trim Then
                            cbLLight.SelectedIndex = i
                            Exit For
                        End If
                    Next
                    For i As Integer = 0 To cbLChTag.Items.Count - 1
                        If cbLChTag.Items(i).ToString = asngLV.SelectedItems(0).SubItems(1).Text.Remove(asngLV.SelectedItems(0).SubItems(1).Text.IndexOf("@")).Trim Then
                    ...
                    For Each l As KeyValuePair(Of Channel, Integer) In s.LightList
                        If l.Key.Name = asngLV.SelectedItems(0).SubItems(1).Text.Remove(asngLV.SelectedItems(0).SubItems(1).Text.IndexOf("@")).Trim And _
                           l.Key.ParentLight = asngLV.SelectedItems(0).Text.Remove(0, asngLV.SelectedItems(0).Text.IndexOf(":") + 1).Trim Then
        ...
        Private Sub cmbChTag_TextChanged(sender As Object, e As EventArgs) Handles cbLChTag.SelectedIndexChanged
            If cbLChTag.Text <> "" And asngLV.SelectedIndices.Count = 1 Then
                Dim temp As String = asngLV.SelectedItems(0).SubItems(1).Text.Remove(0, asngLV.SelectedItems(0).SubItems(1).Text.IndexOf("@")).Trim
        ...
        Private Sub saveChannels()
            ...
            End If
        End Sub
        Private Sub SaveCues()
            IO.File.WriteAllText("Submasters", "[Submasters]" & vbCrLf)
            IO.File.WriteAllText("Cues", "[Cues]" & vbCrLf)
            IO.File.WriteAllText("Sfx", "[Sfx]" & vbCrLf)
            IO.File.WriteAllText("Projection", "[Projection]" & vbCrLf)
            For Each s As String In omittedCueList
                CurProject.Settings.SetSetting("Submasters", s, GetSubmasterByName(s).ToString)
                CurProject.Settings.SetSetting("Cues", s, GetCueByName(s).ToString)
                CurProject.Settings.SetSetting("Sfx", s, GetSfxCueByName(s).ToString)
                CurProject.Settings.SetSetting("Projection", s, GetPjxCueByName(s).ToString)
            Next
            For Each i As ListViewItem In lv.Items
                CurProject.Settings.SetSetting("Submasters", i.Text, GetSubmasterByName(i.Text).ToString)
                CurProject.Settings.SetSetting("Cues", i.Text, GetCueByName(i.Text).ToString)
                CurProject.Settings.SetSetting("Sfx", i.Text, GetSfxCueByName(i.Text).ToString)
                CurProject.Settings.SetSetting("Projection", i.Text, GetPjxCueByName(i.Text).ToString)
            Next
        End Sub
            ...
            If asngLV.SelectedItems.Count = 1 Then
                ...
                Else
                    ...
                    If Not asngLV.SelectedItems(0).Text.StartsWith("L") Then
                        ...
                        Else
                            MsgBox("No lights are available for programming!", MsgBoxStyle.Information Or _
                                                                               MsgBoxStyle.MsgBoxSetForeground, "No Lights Available")
                            cbType.SelectedIndex = 1
                        End If
                    End If
                End If
        ...
        Private Sub DiagViewCue_Paint(sender As Object, e As PaintEventArgs) Handles MyBase.Paint
        ...
        Private Sub btnFader_Click(sender As Object, e As EventArgs) Handles btnFader.Click
            FaderBar.Top = channelsEditor.Top + btnFader.Top - FaderBar.Height
            If asngLV.SelectedItems.Count > 0 Then
                ...
                Else
                    ...
                    If asngLV.SelectedItems(0).Text.StartsWith("C") Then
                        For Each c As KeyValuePair(Of Integer, Integer) In s.ChannelList
                            If c.Key = CInt(unpadZeros(asngLV.SelectedItems(0).Text.Remove(0, asngLV.SelectedItems(0).Text.IndexOf(":") + 1).Trim)) Then
                    ...
                    Else
                        For Each l As KeyValuePair(Of Channel, Integer) In s.LightList
                            Dim temp As String = asngLV.SelectedItems(0).SubItems(1).Text.Remove(asngLV.SelectedItems(0).SubItems(1).Text.IndexOf("@")).Trim
                            If l.Key.Name = temp And _
                               l.Key.ParentLight = asngLV.SelectedItems(0).Text.Remove(0, asngLV.SelectedItems(0).Text.IndexOf(":") + 1).Trim Then
                        ...
                        Next
                    End If
                    Cursor.Position = New Point(CInt(editor.Left + pnlLFX.Left + FaderBar.Left + FaderBar.Width / 2), _
                    ...
                    For Each l As KeyValuePair(Of Channel, Integer) In s.LightList
                        If l.Key.Name = temp And _
                           l.Key.ParentLight = asngLV.SelectedItems(0).Text.Remove(0, asngLV.SelectedItems(0).Text.IndexOf(":") + 1).Trim Then
                            ...
                            asngLV.SelectedItems(0).SubItems(1).Text = temp & " @ " & CStr(Math.Round(FaderBar.Value / 25.5, 1))
                            Exit For
                        End If
                    Next
                End If
        ...
        Private Sub tb_KeyPress(sender As Object, e As KeyPressEventArgs) Handles tbFadeTime.KeyPress, tbFadeDelay.KeyPress, _
                                                                                  tbSDelay.KeyPress, tbPDelay.KeyPress, tbPJXDelay.KeyPress, tbPJXDur.KeyPress, tbAllDelay.KeyPress, tbAllFade.KeyPress
        ...
        Private Sub tbDuration_Leave(sender As Object, e As EventArgs) Handles tbFadeTime.Leave, tbPJXDur.Leave
            ...
            End If
        End Sub
        Private Sub tbDelay_Leave(sender As Object, e As EventArgs) Handles tbPDelay.Leave, tbSDelay.Leave, tbPJXDelay.Leave, tbFadeDelay.Leave, _
        ...
        Private Sub DiagViewCue_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        ...
        Private Sub tabLFX_Click(sender As Object, e As EventArgs) Handles tabLFX.Click
            ...
            tabPJX.Tag = Nothing
            SelTab(tabPJX, False)
            Dim curcue As CueSys.Cue = GetCurCue()
            My.Settings.favCueTab = 1
            My.Settings.Save()
            pnlLFX.Show()
            pnlSFX.Hide()
        ...
        Private Sub tab_EnabledChanged(sender As Object, e As EventArgs) Handles tabSFX.EnabledChanged, tabPJX.EnabledChanged, tabLFX.EnabledChanged
        ...
        Private Sub cbPJX_CheckedChanged(sender As Object, e As EventArgs) Handles cbPJX.CheckedChanged
            ...
            If cbPJX.Checked Then
                If curTab = CuesTab.Pjx Then tabPJX.Tag = 1 Else tabPJX.Tag = Nothing
                tabPJX.Enabled = True
                If curTab = CuesTab.None Then
                    curTab = CuesTab.Pjx
                    tabLFX.Tag = Nothing
        ...
        Private Sub lbWarning_VisibleChanged(sender As Object, e As EventArgs) Handles lbWarning.VisibleChanged
            lbWarning.Top = tbName.Top - lbWarning.Height - 1
                    ...
                    If .ShowDialog = DialogResult.OK Then
                        For Each it As ListViewItem In lvToPlay.Items
                            If it.Text = .Selected Then
                                MsgBox("Warning: This resource is already set to be played at this cue. The same resource cannot be played " & _
                                       "twice in one cue.", MsgBoxStyle.MsgBoxSetForeground, "Warning")
                                Exit Sub
                            End If
                        Next
        ...
        Private Sub btnSAdd_Click(sender As Object, e As EventArgs) Handles btnSAdd.Click
            ...
            Using diag As New DiagSelectAudio
                With diag
                    ...
                    If .ShowDialog = DialogResult.OK Then
                        For Each it As ListViewItem In lvToPlay.Items
                            If it.Text = .Selected Then
                                MsgBox("Warning: This resource is already set to be stopped at this cue. You cannot stop a stopped resource.", _
                                       MsgBoxStyle.MsgBoxSetForeground, "Warning")
                                Exit Sub
                            End If
                        Next
            ...
            End Using
        End Sub
        Private Sub btn_EnableChanged(sender As Object, e As EventArgs) Handles btnSRem.EnabledChanged, btnPRem.EnabledChanged, _
        ...
        'Private Sub lvSFX_ItemActivate(sender As Object, e As EventArgs) Handles lvToPlay.ItemActivate, lvToStop.ItemActivate
        '    Dim sendLV As ListView = DirectCast(sender, ListView)
        '    If sendLV.SelectedItems.Count = 1 Then
        '        Dim realPath As String
        '        If File.Exists(AFs(cur_respath) & sendLV.SelectedItems(0).Text & ".mp3") Then
        '            realPath = AFs(cur_respath) & sendLV.SelectedItems(0).Text & ".mp3"
        '        ElseIf File.Exists(AFs(cur_respath) & sendLV.SelectedItems(0).Text.ToString & ".wav") Then
        '            realPath = AFs(cur_respath) & sendLV.SelectedItems(0).Text & ".wav"
        '        Else
        '            Exit Sub
        '        End If
        ...
        Private Sub btnPjxPlay_Click(sender As Object, e As EventArgs) Handles btnPjxPlay.Click
            Try
                If lv.SelectedItems.Count = 1 Then
                    GetCurPjxCue.ExecuteCue(pbPjx)
                End If
            Catch ex As Exception
                MsgBox("Error occurred while attempting to play an image transition effect (Cue -> PJX). Details displayed below." _
                       & vbCrLf & ex.ToString, MsgBoxStyle.MsgBoxSetForeground, "Error")
            End Try
        End Sub
        ...
        Private Sub cbPjxEffect_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbPjxEffect.SelectedIndexChanged
            Dim curPjx As PjxCue = GetCurPjxCue()
        ...
        Private Sub DiagViewCue_MouseDown(sender As Object, e As MouseEventArgs) Handles MyBase.MouseDown, title.MouseDown
            _moving = True
            _ppt = e.Location
        End Sub

        Private Sub DiagViewCue_MouseMove(sender As Object, e As MouseEventArgs) Handles Me.MouseMove, title.MouseMove
            If _moving Then
                Me.Location = New Point(Me.Left + e.X - _ppt.X, Me.Top + e.Y - _ppt.Y)
            End If
        End Sub

        Private Sub DiagViewCue_MouseUp(sender As Object, e As MouseEventArgs) Handles Me.MouseUp, title.MouseUp
            _moving = False
        End Sub

        Private Sub DiagViewCue_VisibleChanged(sender As Object, e As EventArgs) Handles MyBase.VisibleChanged
            If Me.Visible Then
                loadLv()
            End If
        End Sub
